...
- **`backlog.md`**: The product backlog, roadmap, and pending tasks. The "Session Focus" at the top dictates immediate priorities.
...
2. Silently read the top of `.agents/memory/backlog.md` to check the "Session Focus".
3. If the backlog contains tasks, proactively summarize the "Session Focus" to the user and ask if they are ready to begin, or if they would prefer to run `/plan` to groom the backlog. Else, propose to run `/plan`.
